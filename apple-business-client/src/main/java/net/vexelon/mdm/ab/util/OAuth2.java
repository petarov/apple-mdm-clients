 ...
 *
 * @see <a href="https://developer.apple.com/documentation/apple-school-and-business-manager-api/implementing-oauth-for-the-apple-school-manager-and-apple-business-api">
...
public final class OAuth2 {
	...
	/**
	 * Generates and signs a client-assertion JWT (ES256) ready for use in an OAuth2 token request.
	 *
	...
	public String createClientAssertion() {
		...
		String header = toBase64(new TreeMap<>(Map.of("alg", "ES256", "kid", keyId)));

		String payload = toBase64(new TreeMap<>(
				Map.of("aud", AUDIENCE, "exp", nowInSeconds + tokenValidity.toSeconds(), "iat", nowInSeconds, "iss",
						teamId, "jti", UUID.randomUUID().toString(), "sub", clientId)));
	...
	private String toBase64(@Nonnull Map<?, ?> map) {
		try {
	...
	private byte[] sign(@Nonnull String signingInput) {
		try {
			// re-encode through the JDK KeyFactory so that keys loaded via BouncyCastle (which are BC-provider-bound)
			// become SunEC keys compatible with SHA256withECDSAinP1363Format.
			// @see also - https://stackoverflow.com/a/77654297
